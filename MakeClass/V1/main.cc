...
// Last-Updated: 四 6月  7 09:56:06 2018 (+0800)
//           By: Hongyi Wu(吴鸿毅)
//     Update #: 15
  ...
  if(argc < 2)
    {
      ...
      std::cout<<"./wu run         mean run the process"<<std::endl;
  ...
  if(TString(argv[1]) == "regenerate")
    {
      ...
      chain->MakeClass(Macro);
    }

  if(TString(argv[1]) == "run")
    {
      UserClass *userclass = new UserClass(chain);
      userclass->Loop();
    }
